...
export function GameProvider({ children }: { children: ReactNode }) {
  const adapterRef = useRef(createLocalStorageAdapter());
  const [db, setDb] = useState<GameDb>(() => loadOrCreateGame(adapterRef.current));
  ...
  const restart = useCallback(() => {
    db.reset();
    setDb(loadOrCreateGame(adapterRef.current));
    setVersion((v) => v + 1);
  }, [db]);
  ...
  const value = useMemo<GameContextValue>(() => {
    ...
    const playerFighter = world.playerFighterId
      ...
      : undefined;
    return { db, world, version, commit, updateWorld, playerFighter, restart };
  ...
  }, [db, version, commit, updateWorld, restart]);
