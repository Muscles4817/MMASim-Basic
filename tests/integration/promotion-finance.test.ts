...
describe('medical suspensions are real', () => {
  ...
  it('never books a fighter inside their own suspension', () => {
     ...
     * test tier inside `npm run typecheck` (docs/19 §7.4 F4), and reviving it failed on the
     * first run — not because the suspension was ignored but because the world was writing bouts
     * to fighters' records twice (F7, fixed, guarded in `living-world.test.ts`).
     *
     * With that fixed it holds outright: over a simulated year, no fighter takes a bout fewer
     * than 47 days after being stopped, against a 30-day commission minimum.
     */
    const db = game();
    const player = (db.fighters.findAll() as Fighter[])[0]!;
    advanceWorld(db, 0, 365, player.id);
