...
describe('the cut model, taken apart', () => {
  ...
  it('prints the ceiling the model imposes, as a fraction of walking weight', () => {
    say('\n\n═══ What the model will let anybody lose ═══');
    say(
      '\n  floor / walking = (1 − bodyFat) / (1 − campFat) × (1 − transient), so the maximum cut a\n' +
        '  body can make depends on exactly two of its own numbers: how fat it is, and how much\n' +
        '  water it tolerates losing. Nothing else in the chain moves it.\n',
    );
  ...
  it('exonerates the composition inference: the split cannot move the floor at fixed size', () => {
     ...
     * on the reasoning that `physiqueForMeasurements` splits lean mass evenly between frame and
     * muscle and that a short dense fighter is mis-served by it. This is the check that settles the
     * question rather than arguing it, and the first draft of the check was itself wrong in a way
     * worth recording: it moved `frameIndex` to 90 and asserted the floor did not budge, and the
     * floor moved forty pounds. Of course it did. Lean mass is *derived* from frame and muscle, so
     * raising one builds a different, larger man — his floor should move.
     *
     * The claim that actually matters is narrower. Hold the fighter's **measured** height and
     * walking weight, which is what a calibration entry states, and redistribute lean mass between
     * frame and muscle at constant total. The floor does not move at all, because
     * `floor = lean / (1 − campFat) × (1 − transient)` and lean is pinned by the measurements. So
     * however that split is made — evenly, or all-frame, or all-muscle — the verdict is identical,
     * and `compositionInference` cannot be why anybody is rejected. Those two filings are wrong.
     */
    const c = CASES.find((x) => x.name === 'Alex Pereira')!;
    const above = massCoefficient(c.body) - 9.5; // male base
    for (const frameShare of [0, 0.25, 0.5, 0.75, 1]) {
      const frameIndex = ((above * frameShare) / 2.6) * 100;
      const muscleIndex = ((above * (1 - frameShare)) / 3.2) * 100;
      if (frameIndex > 100 || muscleIndex > 100) continue;
      const split = { ...c.body, frameIndex, muscleIndex } as Body;
      expect(massCoefficient(split)).toBeCloseTo(massCoefficient(c.body), 9);
      expect(cutChain(split).weighInFloorLbs).toBeCloseTo(cutChain(c.body).weighInFloorLbs, 9);
      expect(cutChain(split).walkingWeightLbs).toBeCloseTo(cutChain(c.body).walkingWeightLbs, 9);
    }
  });
