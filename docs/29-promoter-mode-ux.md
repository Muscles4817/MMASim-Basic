...
## The generated-world defect, since fixed

Promoter mode could not book anybody on a _generated_ world — which since doc 27 is the default a
new player gets. It was not caused by this rework (on the same commit, the old card builder filled
**0 of 9 slots** there too), but the planning screens surfaced it starkly rather than quietly, which
is how it got found.

`generateWorld` built the population _at_ the era's start date, ran eight years of pre-history past
it, and wound the clock back — and the clock was the only thing that wound back. `readyOnDay` is an
absolute day stamped after every loss, so nine fighters in ten opened the game serving a suspension
that ended years later, and every matchmaking path in the codebase filters on it.

Fixed by doing what doc 27 § 4.2 already specified: build the population before the start date and
simulate forward onto it. See **doc 27 § 11a** for the measurements, the second half of the fix (a
population built to be aged), and the invariants now pinned against it.
